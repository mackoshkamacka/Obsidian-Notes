...
- If the bell rings, $p$, or the flag drops, $q$, then the race is over $r$. 
  $\therefore$  If $\neg r$, then neither $p$ nor $q$ occurred.
- If $x=2$, $p$, or $x=-2$, $q$, then $x^2$=4, $r$. 
  $\therefore$  If $\neg r$, then neither $p$ nor $q$ occurred. 
...
- Tautology is a statement form that is always true. ($p\vee \neg p \equiv T$)
- A contradiction is a statement form that is always false. ($p \wedge \neg p \equiv F$)
- Two statement forms are called logically equivalent if, and only if they have identical truth tables: 
 - De Morgan's Law: $\neg (p \wedge q) \equiv (\neg p \vee \neg q)$ 

![[Pasted image 20250120201920.png]]
### Truth Tables 
...
Since each position of the row has two possible values (true and false) there will be 2 rows for each proposition. Thus, the number of propositions, $n$ causes $2^n$ cases for the compound statements. When dealing with binary, starting with F F F F ... = 0 can be useful as each row increments by 1. 
